...
sum of its own digits, each raised to the power of the number of digits in
a given base. In this Kata, we will restrict ourselves to decimal (base 10).
...
def narcissistic(value):
    nums = list(str(value))
    return sum([int(x) ** len(nums) for x in nums]) == value
